...
export async function getNightLeaderboard(
  nightId: string
): Promise<NightResult[]> {
  const rows = await db
    .select({
    ...
    .innerJoin(
      userProfiles,
      eq(pokerNightResults.userId, userProfiles.id)
    )
...
export async function getGroupLeaderboard(
  ...
  if (dateFrom) {
    conditions.push(gte(pokerNights.date, dateFrom));
  }
  if (dateTo) {
    conditions.push(lte(pokerNights.date, dateTo));
  }
  ...
  const rows = await db
    ...
    .innerJoin(
      pokerNights,
      eq(pokerNightResults.nightId, pokerNights.id)
    )
    .innerJoin(
      userProfiles,
      eq(pokerNightResults.userId, userProfiles.id)
